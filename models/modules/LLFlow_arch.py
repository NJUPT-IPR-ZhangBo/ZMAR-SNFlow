...
class LLFlow(nn.Module):
    ...
    def normal_flow(self, gt, lr, mask, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
        ...
        #        mean = squeeze2d(lr_enc(['color_map'], 8) if random.random() > self.opt['train_gt_ratio'] else squeeze2d(gt/(gt.sum(dim=1, keepdims=True) + 1e-4), 8)
        #    else:
    ...
    def rrdbPreprocessing(self, lr, mask):
        ...
        # block_idxs =  []
        # if len(block_idxs) > 0:
        # low_level_features=rrdbResults[block_{1，3，5，7}]，所以concate把这些特征拼在一起了64*4=256
        # low_level_features = [rrdbResults["block_{}".format(idx)] for idx in [2]]
        # low_level_features.append(rrdbResults['color_map'])
        concat = rrdbResults["cat_f"]
        # concat = torch.cat(low_level_features, dim=1)#这里面可以把Unet的平层特征拼接在一起

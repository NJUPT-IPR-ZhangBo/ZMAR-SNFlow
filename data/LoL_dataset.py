...
class LoL_Dataset_v2(data.Dataset):
    ...
    def load_pairs(self, folder_path):
        low_list = os.listdir(os.path.join(folder_path, 'low_mask1' if self.train else 'low_mask'))
        ...
        # mask_list = os.listdir(os.path.join(folder_path, 'MASK' if self.train else 'mask'))
        ...
        for idx in range(len(low_list)):
            ...
            pairs.append(
                [cv2.cvtColor(
                    cv2.imread(os.path.join(folder_path, 'low_mask' if self.train else 'low_mask', f_name_low)),
                    cv2.COLOR_BGR2RGB),  # [:, 4:-4, :],
                 cv2.cvtColor(cv2.imread(os.path.join(folder_path, 'high' if self.train else 'high', f_name_high)),
                              cv2.COLOR_BGR2RGB),  # [:, 4:-4, :],
    ...
    def __getitem__(self, item):
        ...
        # imgh, imgw = lr.shape[0:2]
        # mask = cv2.resize(mask, (imgw, imgh))
        # image_mask = lr + mask
        # lr = np.where(image_mask >= 255, 0, lr)
        ...
        threshold = 0.1
        rg_difference = R_split / (R_split + G_split + B_split + 0.0001)
        rb_difference = G_split / (R_split + G_split + B_split + 0.0001)
        gb_difference = B_split / (R_split + G_split + B_split + 0.0001)

        rg_difference[rg_difference <= threshold] = 0
        rb_difference[rb_difference <= threshold] = 0
        gb_difference[gb_difference <= threshold] = 0

        zero_array = rg_difference * rb_difference * gb_difference
        ...
        return {'LQ': lr, 'GT': hr, 'MASK': mask, 'LQ_path': f_name, 'GT_path': f_name}
